...
<article  class="md:px-5 lg:px-0 w-full md:w-[70%] md:mx-auto mt-10 md:mt-0">
    ...
    <div class="mt-5 2xl:mt-10 max-w-[900px] dark:bg-[#1E1F23] border-2 border-blue-300/20 dark:border-slate-700 mx-auto overflow-hidden  rounded-2xl shadow-md ">
                ...
                @foreach ($admins as $admin)
                    <div class="shadow-md dark:shadow-slate-900 p-3 border border-blue-300/50 dark:border-slate-700 rounded-xl justify-between flex gap-x-3 items-center">
                        ...
                        <div class="parentNode flex items-center gap-x-4 md:gap-x-6">
                            <input id="userId" type="hidden" value="{{ $admin->id }}">
                            <div class="flex items-center gap-x-2">

                                {{-- profile  --}}

                                @if (Auth::user()->id != $admin->id)
                                    <a href="{{ route('member#accountProfilePage',$admin->id) }}"  class="text-white bg-blue-400 hover:bg-blue-500 duration-300 rounded-full w-7 h-7 md:w-8 2xl:w-9 md:h-8 2xl:h-9 flex justify-center items-center shadow-md" title="Profile">
                                ...
                                {{-- //message  --}}
                                @if (Auth::user()->id != $admin->id)
                                    <a href="{{ route('chat#messagePage',$admin->id) }}"  class="text-white bg-violet-400 hover:bg-violet-500 duration-300 rounded-full w-7 h-7 md:w-8 2xl:w-9 md:h-8 2xl:h-9 flex justify-center items-center shadow-md" title="Message">
                                ...
                                @if (Auth::user()->role == 'admin')
                                    @if (Auth::user()->id != $admin->id)
                                    <a href="{{ route('admin#deleteAccount',$admin->id) }}" class="text-white bg-red-400 hover:bg-red-500 duration-300 rounded-full w-7 h-7 md:w-8 2xl:w-9 md:h-8 2xl:h-9 flex justify-center items-center shadow-md" type="button" title="Delete">
                            ...
                            @if (Auth::user()->role == 'admin')
                                @if (Auth::user()->id != $admin->id)
                                    <select class="roleChange focus:outline-none focus:bg-blue-500 bg-blue-400 text-white px-2 md:px-3 py-1 rounded-xl shadow-md md:text-sm text-xs" name="" id="">
                                        ...
                                        <option value="member" @if ($admin->role == 'member') selected   @endif >Member</option>
                                    </select>
                                @endif
                            @endif
                        </div>
        ...
        <!-- user list with grid  -->
        <div id="gridDisplay" class="hidden bg-[#f6f8fc] dark:bg-[#1E1F23] h-[650px] md:h-[500px] max-h-[700px] md:max-h-none
        2xl:h-[700px] overflow-y-scroll p-3 md:p-5 2xl:p-10">
                ...
                <!-- card list  -->
                    @foreach ($admins as $admin)
                        <div class="border dark:shadow-slate-900 bg-slate-50 dark:bg-[#1E1F23] dark:border-slate-700 border-blue-300/50 shadow-md rounded-2xl text-center md:pt-5 px-0 p-4 2xl:p-8">
                            <div class="flex justify-center @if(Auth::user()->id == $admin->id) 2xl:mt-5 @endif">
                                <a @if (Auth::user()->id == $admin->id)
                                    href="{{ route('account#profilePage',$admin->id) }}"
                                    @else
                                        href="{{ route('member#accountProfilePage',$admin->id) }}"
                            ...
                            <div class="mt-5 parentNode">
                                ...
                                <div class="">
                                    <a @if (Auth::user()->id == $admin->id)
                                        ...
                                        @else
                                            href="{{ route('member#accountProfilePage',$admin->id) }}"
                                        @endif class="text-slate-700 text-sm md:text-md 2xl:text-lg dark:text-slate-200">
                                        {{ $admin->name }}
                                    </a>
                                ...
                                @if (Auth::user()->role == 'admin')
                                    @if (Auth::user()->id != $admin->id)
                                        <select class="roleChange mt-3 md:mt-4 focus:outline-none focus:bg-blue-500 bg-blue-400 text-white px-3 py-1 rounded-xl shadow-md text-xs md:text-sm " >
                                            <option value="admin" @if ($admin->role == 'admin') selected   @endif >Admin</option>
                                            <option value="member" @if ($admin->role == 'member') selected   @endif >Member</option>
                                        </select>
                                    @endif
                                @endif
                                <div class="flex justify-center items-center gap-x-2 md:gap-x-3 mt-3 2xl:mt-5">
                                    {{-- account profile --}}
                                    @if (Auth::user()->id == $admin->id)
                                    <a  href="{{ route('member#accountProfilePage',$admin->id) }}" class="mt-5 text-white bg-blue-400 hover:bg-blue-500 duration-300 text-xs md:text-sm px-3  md:px-5 py-2 rounded-lg flex justify-center items-center shadow-md" title="Profile">
                                       View Your Profile
                                    </a>
                                    @else
                                        <a href="{{ route('member#accountProfilePage',$admin->id) }}"  class="text-white bg-blue-400 hover:bg-blue-500 duration-300 rounded-full w-7 h-7 md:w-9 md:h-9 flex justify-center items-center shadow-md" title="Profile">
                                    ...
                                    {{-- message  --}}
                                    @if (Auth::user()->id != $admin->id)
                                    <a  href="{{ route('chat#messagePage',$admin->id) }}"  class="text-white bg-violet-400 hover:bg-violet-500 duration-300 rounded-full w-7 h-7 md:w-9 md:h-9 flex justify-center items-center shadow-md" title="Message">
                                    ...
                                    @if (Auth::user()->role == 'admin')
                                        @if (Auth::user()->id != $admin->id)
                                            <a  href="{{ route('admin#deleteAccount',$admin->id) }}" class="text-white bg-red-400 hover:bg-red-500 duration-300 rounded-full w-7 h-7 md:w-9 md:h-9 flex justify-center items-center shadow-md" type="button" title="Delete">
